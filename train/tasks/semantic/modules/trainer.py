...
from tasks.semantic.modules.boundary_loss import BoundaryLoss
...
class Trainer():
    def __init__(self, ARCH, DATA, datadir, logdir, path=None):
        ...
        # get the data
        parserModule = imp.load_source("parserModule", '../../tasks/semantic/dataset/' +
        ...
        # self.criterion = OhemCrossEntropy(ignore_index=0, thresh=0.9, min_kept=10000, weight=self.loss_w).to(self.device)
        # self.wce = nn.CrossEntropyLoss(weight=self.loss_w, ignore_index=0, reduction='none').to(self.device)
        self.ls = Lovasz_softmax(ignore=0).to(self.device)
        self.bd = BoundaryLoss(ignore=0).to(self.device)
        ...
        if self.path is not None:
            ...
            w_dict = torch.load(path + "/MetaRangeSeg",
    ...
    def train_epoch(self, train_loader, model, criterion, optimizer, epoch, evaluator, scheduler, color_fn, report=10,
        ...
        for i, (in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, _, _, _) in enumerate(train_loader):
            ...
            output = model(in_vol)
            loss_m = criterion(torch.log(output.clamp(min=1e-8)), proj_labels.long()) + 1.5*self.ls(output, proj_labels.long()) + self.bd(output, proj_labels.long())
